...
export function ResetPasswordForm({
	...
	if (!token) {
		return (
			<div
				className={cn("flex flex-col gap-6 container-custom", className)}
				{...props}
			>
				<Card className="w-full max-w-lg mx-auto card-shadow">
					<CardHeader className="text-center space-y-4">
						...
						<CardTitle className="heading-3 text-ethiopian-red">
							Invalid Reset Link
						</CardTitle>
					...
					<CardContent className="space-y-6">
						<Button
							asChild
							className="w-full h-12 text-base font-semibold btn-primary rounded-lg"
						>
				...
				<Card className="w-full max-w-lg mx-auto card-shadow">
					...
					<CardContent className="space-y-6">
						<Button
							asChild
							variant="outline"
							className="w-full h-12 font-semibold  text-gray-800 hover:bg-gray-700 hover:text-white rounded-lg"
						>
							<Link to="/login" className="inline-flex items-center gap-2">
								<ArrowLeft className="w-4 h-4" />
								Continue to login
	...
	return (
		...
		>
			...
			<div className="text-muted-foreground text-center text-xs text-balance max-w-lg mx-auto">
				...
				<Link
					to="/"
					className="text-ethiopian-green hover:text-ethiopian-green/80 underline underline-offset-4"
				>
					Terms of Service
				</Link>{" "}
				and{" "}
